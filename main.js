...
/**
 * 일일 콘텐츠 로드 (무조건 '내일' 데이터 가져오기 고정 버전)
 */
async function loadDailyContent() {
    ...
    const targetId = `${y}-${m}-${d}`; 

    console.log("🚀 Attempting to fetch shifted content (Tomorrow):", targetId);
    ...
    // 3. Firestore에서 '내일' 날짜 문서 가져오기
    if (window.db) {
        try {
            const docRef = window.db.collection("daily_content").doc(targetId);
            ...
            if (docSnap.exists) {
                ...
                console.log("✅ Success: Shifted content loaded from Firestore for", targetId);
            } else {
                console.warn("⚠️ Firestore Document NOT FOUND:", targetId);
                // 화면에 에러 표시 (디버깅용)
                elements.word.innerHTML = `<span style='color:red;'>[${targetId}] 문서를 Firestore에서 찾을 수 없습니다. (ID 확인 필요)</span>`;
            }
        } catch (error) {
    ...
    if (finalData) {
        ...
        if (cardElements.word) {
            // 날짜 표시는 실제 접속일(오늘)로 표시
            const todayStr = `${now.getFullYear()}.${String(now.getMonth() + 1).padStart(2, '0')}.${String(now.getDate()).padStart(2, '0')}`;
            cardElements.date.textContent = todayStr;
